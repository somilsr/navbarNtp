...
const Birthday = ({setUserBday,birthdayState}) =>{
    ...
    .update(id, firebase.firestore.FieldValue.arrayUnion(
      {
        ...
        wish:"Happy Bday Gate Pass Hi zin"
      }
    ))
  }
  ...
  function formattedDate(dateObj) {
    if (dateObj.getDate() < 10) {
      if (dateObj.getMonth() + 1 < 10)
        return ("0" +(dateObj.getMonth() + 1) + "_0" + dateObj.getDate())
      else
        return ((dateObj.getMonth() + 1) + "_0" + dateObj.getDate())
    } else {
      if (dateObj.getMonth() + 1 < 10)
        return ("0" +(dateObj.getMonth() + 1) + "_" + dateObj.getDate())
      else
        return ((dateObj.getMonth() + 1) + "_" + dateObj.getDate())
    }
  }

  function formattedYear(dateObj){
    return dateObj.getFullYear()
  }

  useEffect(() => {
    const dtObj = new Date(firebase.firestore.Timestamp.now().seconds * 1000)
    const dtObj2 = new Date((firebase.firestore.Timestamp.now().seconds * 1000)+(86400*30*1000))
    ...
    const fullDateToday = formattedYear(dtObj)+"_"+formattedDate(dtObj)
  ...
  return (
    <View style={styles.container}>
      <Text  style={{fontWeight: "bold", fontSize:18, textAlign:"center"}}>Birthdays Today</Text>
      <FlatList
        numColumns={1}
        horizontal={false}
...
function mapStateToProps({ birthdayState}) {
    return {
      birthdayState
    };
}
